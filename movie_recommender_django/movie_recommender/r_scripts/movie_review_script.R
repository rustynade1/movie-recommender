...
movie_review <- function(title, status, review){
  ...
  if (movie_review$status == "Recommended"){
    ...
    reaction <- "That's awesome to hear! Here are movies that are similar to what you just reviewed:"
    ...
    # Display top 5 least similar movies
      ...
      reaction <- "We're sorry to here that. Perhaps these are more to your liking?"
      cat(paste(top_similar_movies,"\n"))
}
    result <- list(reaction,top_similar_movies)
    return (result)
}
